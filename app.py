...
def render_incident_cockpit(site_id: str, api_key: Optional[str]):
    ...
    if prediction_candidates:
        ...
        st.warning(
            ...
            f"推定 **{timeline}** に深刻化、**{affected}台** に影響の可能性"
        )
            ...
            for idx, pred_item in enumerate(prediction_candidates[:3]):
                with radar_cols[idx]:
                    ...
                    st.markdown(
                        ...
                        f"<br><span style='color:#666;'>発生確率（予測: {pred_timeline}）</span>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                    ...
                    st.markdown(f"**将来の影響:** 配下 **{pred_affected}台** が通信断の恐れ")
                        ...
                        if cache_key_analyst in st.session_state.report_cache:
                            ...
                            try:
                                ...
                                if is_pred:
                                    ...
                                    report_scenario = (
                                        f"[予兆検知] Digital Twinが{cand['id']}で障害の前兆を検出。"
                                        f"現在のステータスは「正常」だが、{signal_count}件の微弱シグナルを検出。"
                                        f"推定{pred_timeline}に深刻化し、配下{pred_affected}台に影響の恐れ。\n"
                                        f"検出されたシグナル:\n{pred_reason}\n"
                                        f"このレポートでは、予兆が本当に障害に発展するか確認するための"
                                        f"手動検証手順を提示してください。"
                                    )
        ...
        if selected_incident_candidate and selected_incident_candidate["prob"] > 0.6:
            ...
            if selected_incident_candidate.get('is_prediction'):
                ...
                <div style="background-color:#fff3e0;padding:10px;border-radius:5px;border:1px solid #ff9800;color:#e65100;margin-bottom:10px;">
                    <strong>🔮 Digital Twin 未来予測</strong><br>
                    <b>{selected_incident_candidate['id']}</b> で障害の兆候を検出しました。<br>
                    推定 <b>{timeline}</b> に深刻化、<b>{affected}台</b> のデバイスに影響の可能性があります。<br>
                ...
                if st.button(fix_label):
                    if st.session_state.generated_report is None:
                        ...
                        if is_pred_rem:
                            ...
                            rem_scenario = (
                                f"[予兆対応] {selected_incident_candidate['id']}で障害の前兆を検出。"
                                f"推定{pred_timeline}に深刻化の恐れ（影響{pred_affected}台）。"
                                f"「復旧」ではなく「予防措置」として、障害を未然に防ぐための"
                                f"手順・確認コマンド・エスカレーション基準を提示してください。"
                            )
