...
def render_incident_cockpit(site_id: str, api_key: Optional[str]):
    ...
    # 戻るボタン専用のスタイル（ページ全体に適用）
    st.markdown("""
    <style>
    /* 戻るボタンを赤色にする */
    button[kind="secondary"]:has(p:contains("一覧に戻る")),
    button[data-testid="baseButton-secondary"]:has(p:contains("一覧に戻る")) {
    ...
    with col_header[1]:
        if st.button("🔙 一覧に戻る", key="back_to_list", type="primary"):
