...
class AutoTuner:
    """
    Digital Twin Auto-Tuner: forecast_ledger（SQLite）の予兆履歴を分析し、
    閾値チューニング提案を生成する。

    データフロー:
      forecast_ledger (SQLite)
        → outcome_type: confirmed_incident / mitigated / false_alarm / expired
        → これらを集計して TP/FP/FN を算出
        → 最適閾値をシミュレーション
        → 提案(Proposal)を生成
    """
    ...
    def _load_forecast_data(self, scan_start: float) -> Dict[str, Any]:
        """forecast_ledger から予兆履歴+アウトカムを読み込む"""
        history_map = {}
        outcomes = []
        
        try:
            conn = self.engine.storage._conn
            if not conn:
                return {"history_map": {}, "outcomes": [], "missed": []}
            
            with self.engine.storage._db_lock:
                # 予兆履歴の読み込み
                cur = conn.execute("""
                    SELECT forecast_id, device_id, rule_pattern, confidence,
                           created_at, status, outcome_type, outcome_ts,
                           outcome_note, source, prediction_json
                ...
                for row in cur.fetchall():
                    fid, dev_id, rule_pat, conf, created_at, status, \
                        outcome_type, outcome_ts, outcome_note, source, pred_json = row
                    
                    # 履歴マップに追加
                    history_map[fid] = {
                        "prediction_id": fid,
                        "device_id": dev_id,
                        "rule_pattern": rule_pat or "unknown",
                        "prob": float(conf or 0.5),
                    ...
                    # アウトカムがある場合
                    if outcome_type and outcome_type not in ("", "pending"):
                        # outcome_type を user_action 形式に変換
                        action_map = {
                            "confirmed_incident": OUTCOME_CONFIRMED,
                            "mitigated":          OUTCOME_MITIGATED,
                            "false_alarm":        OUTCOME_FALSE_ALARM,
                            "expired":            OUTCOME_FALSE_ALARM,  # 期限切れ = FP扱い
                        }
                        user_action = action_map.get(outcome_type, outcome_type)
                        
                        outcomes.append({
                            "prediction_id": fid,
                            "user_action": user_action,
                            "timestamp": float(outcome_ts or created_at or 0),
                        })
                
        except Exception as e:
            logger.warning(f"AutoTuner._load_forecast_data: {e}")
        
        return {"history_map": history_map, "outcomes": outcomes, "missed": []}

    def generate_report(self, days: int) -> Dict[str, Any]:
        current_time = time.time()
        scan_start = current_time - (days * 24 * 3600)
        
        # ★ forecast_ledger（SQLite）からデータ読み込み
        data = self._load_forecast_data(scan_start)
        history_map = data["history_map"]
        outcomes = data["outcomes"]
        
        # ★ engine.history もマージ（JSONベースの古いデータとの互換性）
        for h in self.engine.history:
            ts = h.get("timestamp", 0)
            if ts < (scan_start - 7*86400): continue
            if h.get("suppressed"): continue
            if float(h.get("maintenance_prob", 0.0) or 0.0) >= 0.3: continue
            pid = h.get("prediction_id", "")
            if pid and pid not in history_map:
                history_map[pid] = h
        
        # ★ engine.outcomes もマージ（後方互換性）
        existing_pids = {o["prediction_id"] for o in outcomes}
        for o in self.engine.outcomes:
            if o.get("timestamp", 0) >= scan_start:
                pid = o.get("prediction_id", "")
                if pid and pid not in existing_pids:
                    outcomes.append(o)
                    existing_pids.add(pid)

        missed_incidents = [i for i in self.engine.incident_register 
                          if i.get("timestamp", 0) >= scan_start]
        
        # Filter outcomes
        filtered_outcomes = []
        for out in outcomes:
            if out.get("user_action") in [OUTCOME_SUSPECTED_MAINTENANCE, OUTCOME_PLANNED_MAINTENANCE]: 
                continue
            if out.get("prediction_id") in history_map:
                filtered_outcomes.append(out)
                
        # Filter missed
        filtered_missed = [inc for inc in missed_incidents 
                          if not inc.get("suppressed_by_maintenance")]
        ...
        for out in filtered_outcomes:
            ...
            if rule not in rule_stats:
                rule_stats[rule] = {"scores_tp": [], "scores_fp": [], "missed_count": 0, 
                                    "prediction_count": 0, "labeled_count": 0}
        ...
        for h in history_map.values():
            if h.get("timestamp", 0) >= scan_start:
                r = h.get("rule_pattern", "unknown")
                if r in rule_stats: 
                    rule_stats[r]["prediction_count"] += 1

        # ★ 統計情報（デバッグ・UI表示用）
        total_predictions = len(history_map)
        total_outcomes = len(filtered_outcomes)
        total_with_outcome = sum(1 for h in history_map.values() 
                                if h.get("status") not in ("open", None, ""))

        proposals = []
        MIN_SAMPLES = 3  # ★ デモ環境用: 最低3サンプル（本番では5-30推奨）
        ...
        for rule_ptrn, stats in rule_stats.items():
            ...
            proposals.append({
                ...
                "expected_impact": {
                    ...
                    "fn_delta": int(best_fn - curr_fn_count) if 'best_fn' in locals() else 0,
        ...
        return {
            ...
            "stats": {
                ...
                "total_outcomes_labeled": total_outcomes,
