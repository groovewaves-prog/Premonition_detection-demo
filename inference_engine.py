...
class LogicalRCA:
    ...
    def analyze(self, alarms: List) -> List[Dict[str, Any]]:
        ...
        # ==========================================================
        # ★ INFOアラームのみのデバイスはprob制限（予兆シグナル保護）
        # ==========================================================
        # severity=INFO のアラームのみで構成されるデバイスが Gemini によって
        # CRITICAL と過剰判定された場合、WARNING レベル (prob=0.70) に制限する。
        # 設計意図: INFOレベルの予兆シグナルは「障害確定」ではなく
        # Digital Twin の予兆検知パイプラインで処理されるべきである。
        ...
        # ==========================================================
        # ★ Digital Twin: 予兆検知 (Predictive Analysis)
        # ==========================================================
        # WARNING 結果を起点に、未来の障害をシミュレートして追加する。
        # app.py は prob > 0.5 の結果を root_cause_candidates に含める
        # ため、予兆アラートは既存UIを変更せず自動的に表示される。
        if self.digital_twin is not None:
            try:
                predictions = self.digital_twin.predict(
                    analysis_results=results,
                    msg_map=msg_map,       # L229 で構築済みの既存変数を再利用
                    alarms=alarms,
                )

                if predictions:
                    # 既に CRITICAL の結果がある device は予測不要
                    critical_ids = {
                        r["id"] for r in results if r.get("prob", 0) >= 0.9
                    }
                    filtered = [
                        p for p in predictions
                        if p["id"] not in critical_ids
                    ]

                    # WARNING 結果を予測で置換（両方表示は冗長）
