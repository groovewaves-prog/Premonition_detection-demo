...
def generate_alarms_for_scenario(topology: dict, scenario: str) -> List[Alarm]:
    ...
    if "[FW]" in scenario:
        ...
        if fw_ids:
            if "電源障害：両系" in scenario:
                # 両系障害：冗長グループ全体がダウン、配下すべて影響
                for fw_id in fw_ids:
                    alarms.append(Alarm(fw_id, "Power Supply: Dual Loss (Device Down)", "CRITICAL", is_root_cause=True))
                
                # 冗長グループ全体の配下を取得
                downstream = _get_all_downstream_devices(topology, fw_ids)
                for dev_id in downstream:
                    alarms.append(Alarm(dev_id, "Device Unreachable", "CRITICAL", is_root_cause=False))
            
            elif "電源障害：片系" in scenario:
                # 片系障害：1台のみ、配下影響なし（HA冗長で継続）
                alarms.append(Alarm(fw_ids[0], "Power Supply 1 Failed", "WARNING", is_root_cause=True))
            elif "FAN" in scenario:
                alarms.append(Alarm(fw_ids[0], "Fan Fail", "WARNING", is_root_cause=True))
            elif "メモリ" in scenario:
                alarms.append(Alarm(fw_ids[0], "Memory High", "WARNING", is_root_cause=True))
        return alarms
