...
def build_radar_svg(score_breakdown: Dict[str, float], size: int = 220) -> str:
    keys = ["semantic", "trend", "volatility", "history", "interaction"]
    ...
    for i, k in enumerate(keys):
        ...
        labels += (
            f'<text x="{lx:.1f}" y="{ly:.1f}" font-size="9" text-anchor="middle" '
            f'fill="{color}" font-family="sans-serif">{lbl}</text>'
            f'<text x="{lx:.1f}" y="{ly+10:.1f}" font-size="9" text-anchor="middle" '
...
def render_explanation_panel(
    ...
    with st.expander(f"🔬 AI証拠パネル  [{badge_label}]", expanded=expanded):
        ...
        # ── レーダーチャート ──
        if score_breakdown:
            svg = build_radar_svg(score_breakdown, size=210)
