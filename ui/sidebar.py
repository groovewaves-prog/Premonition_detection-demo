...
def _render_weak_signal_injection():
    ...
    with st.expander("🔮 予兆シミュレーション", expanded=True):
        ...
        # Session State に保存
        if log_messages:
            # ★ 障害シナリオとの競合チェック（全拠点を対象に確認）
            # active_site だけでなく、全拠点のシナリオを確認することで
            # シナリオ切り替え直後の再注入を防ぐ
            conflict_site = None
            conflict_scenario = None
            for _sid in list_sites():
                _scen = st.session_state.site_scenarios.get(_sid, "正常稼働")
                if _scen != "正常稼働":
                    conflict_site = _sid
                    conflict_scenario = _scen
                    break

            if conflict_site:
                st.error(
                    f"⛔ **競合エラー**\n\n"
                    f"現在、拠点 `{conflict_site}` では障害シナリオ「**{conflict_scenario}**」が実行中です。\n"
