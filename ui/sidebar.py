...
def _render_llm_backend_selector():
    ...
    設定は st.session_state["llm_config"] に保存され、
    cockpit.py が DTE 初期化時に参照する。

    ハイブリッドルーティング:
      標準スコアリング → Ollama（ローカル）優先
      メンテ計画文・疑似データ生成 → Google Gemma API 固定（gemma-3-12b-it）
    """
    if "llm_config" not in st.session_state:
        st.session_state["llm_config"] = {
            "backend":       "ollama",    # デフォルト: ハイブリッド（Ollama+Google Gemma）
            "ollama_url":      "http://localhost:11434",
            "ollama_model":  "gemma3:12b",          # Ollama での gemma-3-12b-it タグ
            "google_key":    os.environ.get("GOOGLE_API_KEY") or os.environ.get("GEMINI_API_KEY"),
            "google_model":  "gemma-3-12b-it",      # Google AI Studio のモデル名
        }
    ...
    with st.expander("🤖 LLM バックエンド設定", expanded=False):
        backend = st.radio(
            ...
            format_func=lambda x: (
                "☁️ Google Gemma API（クラウド）"
                if x == "google"
                else "🏠 Ollama（ローカル）+ Google Gemma エッジ処理"
            ),
            index=0 if cfg.get("backend") == "google" else 1,
        ...
        st.caption(
            "💡 **ハイブリッド動作:** ローカルモードでも「メンテ計画文生成」など"
            "品質要求の高いタスクは自動的に **Google Gemma-3-12b-it API** を使用します。"
        )
        ...
        if backend == "ollama":
            ...
            _recommended = ["gemma3:12b", "gemma3:27b", "gemma3:4b", "qwen2.5:7b", "llama3.1:8b"]
            _cached_models = st.session_state.get("_ollama_model_list", [])
            all_opts = list(dict.fromkeys(_recommended + _cached_models))
            cur_model = cfg.get("ollama_model", "qwen2.5:7b")
            if cur_model not in all_opts:
                all_opts.insert(0, cur_model)

            ollama_model = st.selectbox(
                "モデル",
                all_opts,
                index=all_opts.index(cur_model) if cur_model in all_opts else 0,
                key="ollama_model_select",
                help="推奨: gemma3:12b（Google gemma-3-12b-it のローカル版）",
            )
            cfg["ollama_model"] = ollama_model

            # 接続テスト
            if st.button("🔌 接続テスト", key="ollama_ping_btn"):
                with st.spinner("Ollama に接続中..."):
                    try:
                        from digital_twin_pkg.llm_local import test_ollama_connection
                        res = test_ollama_connection(
                            base_url=ollama_url,
                            model=ollama_model,
                        )
                        st.session_state["_ollama_test"] = res
                        if res.get("models"):
                            st.session_state["_ollama_model_list"] = res["models"]
                    except Exception as e:
                        st.session_state["_ollama_test"] = {
                            "connected": False, "error": str(e),
                            "model_installed": False, "inference_ok": False, "inference_ms": 0,
                        }

            test = st.session_state.get("_ollama_test")
            if test:
                if test.get("inference_ok"):
                    st.success(f"✅ 接続・推論 OK（{test.get('inference_ms')}ms）")
                elif test.get("connected") and test.get("model_installed"):
                    st.warning("⚠️ 接続OK・推論テスト失敗")
                elif test.get("connected"):
                    st.warning(
                        f"⚠️ 接続OK・モデル未インストール\n\n"
                        f"`ollama pull {ollama_model}`"
                    )
                else:
                    st.error(f"❌ {test.get('error', '接続失敗')}")
                if test.get("models"):
                    st.caption(f"検出モデル: {', '.join(test['models'][:5])}")
            ...
            model_sel = st.selectbox(
                ...
                help="gemma-3-12b-it: バランス型（本システム標準）",
            )
            cfg["google_model"] = model_sel
            st.caption(
                "ℹ️ **全タスク** に Google Gemma API を使用します。"
                " API使用量が増える場合があります。"
            )

        # 変更検出 → エンジン再初期化
        _prev = st.session_state.get("_llm_cfg_prev")
        _cur  = f"{cfg.get('backend')}|{cfg.get('ollama_model')}|{cfg.get('google_model')}"
